...
def extract_epoch_from_filename(file_path):
    ...
    try:
        ...
        epoch_str = filename.split("_")[-1].replace(".xlsx", "")
...
def extract_max_prices(df, start_time):
    ...
    if df is None or df.empty:
        logger.warning(f"extract_max_prices: No data available. Returning None for all intervals.")
...
def calculate_percentage_changes(original_price, max_prices):
    ...
    if original_price is None or original_price <= 0:
        return [None] * len(max_prices)  # Avoid division by zero

    return [(max_price - original_price) / original_price * 100 if max_price else None for max_price in max_prices]
...
def process_file(file_path, api_key):
    ...
    try:
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc=f"Processing {os.path.basename(file_path)}"):
            token = row["token"]
            original_price = row["price"]  # Get the price from the Excel file
            ...
            # Append results
            updated_data.append(list(row) + max_prices + max_percent_changes)
        ...
        new_columns = max_price_columns + max_percent_columns
...
if __name__ == "__main__":
    input_directory = "./data/"  # Replace with the actual path
